...
traind = pd.read_csv(path + '/source/관광지 추천시스템 Trainset_final_re.csv')
testd = pd.read_csv(path + '/source/관광지 추천시스템 Testset_final_re.csv')
...
#%%
#print(info[info['VISIT_AREA_NM'].str.endswith('집')])
# info = info[~info['VISIT_AREA_NM'].str.endswith('집')]
# info = info[~info['VISIT_AREA_NM'].str.endswith('숙소')]


#%% 
info.to_csv(path + '/source/관광지 추천시스템 여행지 정보 방문 2회 이상_re.csv', index=False)
