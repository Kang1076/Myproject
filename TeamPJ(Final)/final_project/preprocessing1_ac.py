...
# 관광지 선택
visit_area_info = visit_area_info[ (visit_area_info['VISIT_AREA_TYPE_CD'] == 24)]
...
# In[25]: 시/도이름 통일
df['SIDO'].unique()
#%% 시도/군구/읍면에 내 집으로 이런식으로 써진 데이터가 있어서 삭제
df.drop(1901, axis=0, inplace=True)
df.reset_index(drop=True, inplace=True)
#%%
places = list(df['SIDO'])
...
#train set 저장
new_train.to_csv(path + '/source/관광지 추천시스템 Trainset_final_ac.csv', index = False)
#test set 저장
df1.to_csv(path + '/source/관광지 추천시스템 Testset_final_ac.csv', index = False)
