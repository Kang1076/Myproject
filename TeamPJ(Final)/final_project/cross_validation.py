...
Train = pd.read_csv(path + '/source/관광지 추천시스템 Trainset_final.csv')
...
#4번 이상 방문한 곳으로만 필터링
### 여기 숫자 변경
six_places = list(count[count['count']>=2]['VISIT_AREA_NM'])
###
...
def cross_validation(X_train, y_train, cv, iteration, number, learning_rate, depth,
    ...
    for a in tqdm(range(iteration)):
        ...
        ######################################################
        #n_estimators = np.random.choice(np.arange(1600, 2001, 400))
        n_estimators = 400
        print(n_estimators)
        ######################################################
######################################################
...
#초모수 조절
cross_validation(X_train, y_train, cv = 5, iteration = 1, number = 2,
                 learning_rate = 0.02, depth = 6, early_stopping_rounds = 5, random_state = 42)