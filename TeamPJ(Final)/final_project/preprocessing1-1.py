...
#데이터 불러오기
path = 'D:/TeamProject/final_project'
...
#%%
# 데이터 불러오기
new_train = pd.read_csv('D:/TeamProject/final_project/source/관광지 추천시스템 Trainset_final.csv')
pd.set_option('display.max_columns', None)
plt.rcParams['font.family'] = 'D2Coding'

#%%
# 시도/군구별 관광지 방문 횟수 시각화
plt.figure(figsize=(15, 6))
sns.countplot(x='SIDO', data=new_train, order=new_train['SIDO'].value_counts().index)
plt.title('시도/군구별 관광지 방문 횟수')
plt.xlabel('시도/군구')
plt.ylabel('방문 횟수')
...
#%%
# 여행 스타일에 따른 방문 횟수 시각화
plt.figure(figsize=(15, 6))
sns.countplot(x='TRAVEL_MISSION_PRIORITY', data=new_train)
plt.title('여행 스타일에 따른 방문 횟수')
plt.xlabel('여행 스타일 우선순위')
plt.ylabel('방문 횟수')
plt.show()

#%%
# 체류시간과 추천의향의 상관 관계 시각화
plt.figure(figsize=(10, 6))
sns.scatterplot(x='RESIDENCE_TIME_MIN', y='RCMDTN_INTENTION_mean', data=new_train)
plt.title('체류시간과 추천의향의 상관 관계')
plt.xlabel('체류시간 (분)')
plt.ylabel('추천의향 평균')
plt.show()

#%%
# 재방문 여부에 따른 체류시간 분포 시각화
plt.figure(figsize=(10, 6))
sns.boxplot(x='REVISIT_YN_mean', y='RESIDENCE_TIME_MIN', data=new_train)
plt.title('재방문 여부에 따른 체류시간 분포')
plt.xlabel('재방문 여부 (평균)')
plt.ylabel('체류시간 (분)')
plt.show()

#%%
# Train 데이터 불러오기
train_data = pd.read_csv(path + '/source/관광지 추천시스템 Trainset_final.csv')

#%%
# 각 특성에 대한 히스토그램 시각화 예제
plt.figure(figsize=(10, 6))
sns.histplot(train_data['RESIDENCE_TIME_MIN'], bins=30, kde=True, color='skyblue')
plt.title('Distribution of Residence Time')
...
# 여행 스타일에 따른 방문 횟수 시각화
plt.figure(figsize=(10, 6))
sns.countplot(x='TRAVEL_MISSION_PRIORITY', data=train_data)
plt.title('Number of Visits by Travel Style Priority')
plt.xlabel('Travel Style Priority')
plt.ylabel('Number of Visits')
plt.show()

#%%
# 1. 문자열 데이터 제거 또는 수치형으로 변환
train_data = train_data.select_dtypes(include=['float64', 'int64'])  # 수치형 데이터만 선택

#%%
# 2. 문자열을 수치형으로 변환 (해당 열에 문자열이 있을 경우 에러 발생 가능)
